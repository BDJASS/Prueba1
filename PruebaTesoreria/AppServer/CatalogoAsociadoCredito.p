...
/*--------------------------------------------------------------------------  
 File        : CatalogoAsociadocredito:p
               /AsociadoCredito
 Purpose     : .
--------------------------------------------------------------------------*/
...
DEFINE TEMP-TABLE ttAsociados NO-UNDO
    ...
    DEFINE BUFFER bAsociadoCred FOR AsocCred.
...
/* PROCEDIMIENTO: Buscar Asociados de tabla asoccred */
...
--------------------------------------------------------------------------*/
    ...
    FOR EACH bAsociadoCred WHERE bAsociadoCred.Id-Cliente = pIdCliente NO-LOCK:

        /* Buscar al asociado en la tabla Cliente */
        FIND FIRST bCliente WHERE bCliente.Id-Cliente = bAsociadoCred.Id-Asociado NO-LOCK NO-ERROR.

        /* Verificar si está en blkAut */
        FIND FIRST bBlkAut WHERE bBlkAut.Id-Cliente = bAsociadoCred.Id-Asociado NO-LOCK NO-ERROR.
        ...
        CREATE ttAsociados.
        ASSIGN
            ttAsociados.NumCliente     = bAsociadoCred.Id-Cliente
            ttAsociados.NombreCliente  = nombreCliente
            ttAsociados.NumAsociado    = bAsociadoCred.Id-Asociado
